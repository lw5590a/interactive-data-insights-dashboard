...
class DataInsightsDashboard {
    ...
    initializeEventListeners() {
        ...
        const decrease200bps = document.getElementById('decrease200bps');
        if (decrease200bps) decrease200bps.addEventListener('click', () => this.adjustInterestRate(-200));
        
        const decrease100bps = document.getElementById('decrease100bps');
        if (decrease100bps) decrease100bps.addEventListener('click', () => this.adjustInterestRate(-100));
        
        const decrease50bps = document.getElementById('decrease50bps');
        if (decrease50bps) decrease50bps.addEventListener('click', () => this.adjustInterestRate(-50));
        
        const resetRate = document.getElementById('resetRate');
        if (resetRate) resetRate.addEventListener('click', () => this.resetInterestRate());
        
        const increase50bps = document.getElementById('increase50bps');
        if (increase50bps) increase50bps.addEventListener('click', () => this.adjustInterestRate(50));
        
        const increase100bps = document.getElementById('increase100bps');
        if (increase100bps) increase100bps.addEventListener('click', () => this.adjustInterestRate(100));
        
        const increase200bps = document.getElementById('increase200bps');
        if (increase200bps) increase200bps.addEventListener('click', () => this.adjustInterestRate(200));
    ...
    loadDataset(datasetName) {
        ...
        if (datasetName === 'finance') {
            ...
            const originalPortfolioRadio = document.getElementById('originalPortfolio');
            if (originalPortfolioRadio) {
                originalPortfolioRadio.checked = true;
            }
        }
    ...
    updateDatasetDateRange() {
        ...
        // Find date columns
        const dateColumns = Object.keys(this.currentData[0]).filter(key => {
            const value = this.currentData[0][key];
        ...
        // Get all dates from the first date column
...
const performanceObserver = new PerformanceObserver((list) => {
    for (const entry of list.getEntries()) {
        if (entry.entryType === 'navigation') {
            // Page load time monitoring - silent
        }
    }
});
